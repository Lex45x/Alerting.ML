...
namespace Alerting.ML.Sources.Csv
{
    ...
    public class CsvOutagesProvider(string filePath) : IKnownOutagesProvider
    {
        ...
        {
            return outages ?? throw new InvalidOperationException($"{nameof(CsvOutagesProvider)} is not initialized or invalid! Make sure to call {nameof(ImportAndValidate)} before attempting to retrieve outages.");
        }
        ...
        public async Task<ValidationResult> ImportAndValidate()
        {
            ...
            while (!string.IsNullOrWhiteSpace(currentLine))
            {
                ...
                if (rowParts.Length > outageStartIndex && rowParts.Length > outageEndIndex)
                {
                    ...
                    {
                        errorList.Add(new ValidationFailure(nameof(FilePath), $"Line #{lineIndex + 1} contains invalid date time at position {outageStartIndex}."));
                    }

                    if (!DateTime.TryParse(rowParts[outageEndIndex], out var outageEnd))
                    {
                        errorList.Add(new ValidationFailure(nameof(FilePath), $"Line #{lineIndex + 1} contains invalid date time at position {outageEndIndex}."));
                    }

                    if (outageEnd < outageStart)
                    {
                        errorList.Add(new ValidationFailure(nameof(FilePath), $"Line #{lineIndex + 1} has outage start and outage end in invalid order."));
                    }
